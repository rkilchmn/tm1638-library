...
Keypad::Keypad(char *userKeymap, TM1638GCA &keyPad, byte numRows, byte numCols) : _keyPad(keyPad)
{
...
void Keypad::scanKeys()
{
    ...
    byte numScansResults = _keyPad.getKeyScans(scansResults, sizeof(scansResults));

    keyPressed_s keyPressed[8];
    byte numKeyPressed = _keyPad.decodeKeyPressed(scansResults, numScansResults, keyPressed, sizeof(keyPressed));

    // initialize to unpressed
    for (byte c = 0; c < sizeKpd.columns; c++)
    {
        for (byte r = 0; r < sizeKpd.rows; r++)
        {
            bitWrite(bitMap[r], c, false);
        }
    }
    ...
    {
        bitWrite(bitMap[keyPressed[i].KSn - 1], keyPressed[i].Kn - 1, true); 
    }
}
